...
void client() {
	...
	for (int i = 0; i < max_clients; ++i) {
		...
		people.fetch_add(1, std::memory_order_relaxed);
		//people.fetch_add(1, std::memory_order_seq_cst);
...
void teller() {
	...
	while (true) {
		std::this_thread::sleep_for(sleep_time);
		if (people > 0) {
			people.fetch_sub(1, std::memory_order_relaxed);
			//people.fetch_sub(1, std::memory_order_seq_cst);
